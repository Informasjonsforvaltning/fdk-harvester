...
open class KafkaHarvestEventCircuitBreaker(
    ...
    override fun process(record: ConsumerRecord<String, HarvestEvent>) {
        circuitBreaker.executeRunnable {
            ...
            try {
                when (event.phase) {
                    HarvestPhase.INITIATING if event.removeAll != true -> {
                        initiateHarvest(event)
                    }

                    HarvestPhase.INITIATING if event.removeAll == true -> {
                        removeAllResourcesFromSource(event)
                    }

                    HarvestPhase.REMOVING -> {
                        removeSingleResource(event)
                    }
    ...
    private fun initiateHarvest(event: HarvestEvent) {
        ...
        val dataSourceUrl =
            requireNotNull(event.dataSourceUrl?.toString()) {
                "Harvest event missing dataSourceUrl"
            }

        val acceptHeader =
            requireNotNull(event.acceptHeader?.toString()) {
    ...
    private fun dataSourceIdOrNull(event: HarvestEvent): String = event.dataSourceId?.toString() ?: ""
